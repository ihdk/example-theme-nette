 ...
 *
 * Modified version for needs of WpLatte.
 *
 ...
 */
class WpLatteBreadcrumbs
{


	/**
	 * Shows a breadcrumb for all types of pages.  This function is formatting the final output of the
	 * breadcrumb trail.  The self::getItems() function returns the items and this function
	 * formats those items.
	 *
	 * @since 0.1.0
	 * @access public
	...
	public static function breadcrumbs( $args = array() )
	{
		...
		if(function_exists('is_woocommerce') and is_woocommerce()){
			return self::woocommerceBreadcrumbs($args);
		}
	...
	public static function getItems( $args = array() ) {
		...
		elseif ( is_archive() ) {
			...
			elseif ( is_date() ) {
				...
				elseif ( get_query_var( 'w' ) ) {
					...
					if ( is_paged() )
						$trail[] = get_archives_link( add_query_arg( array( 'm' => get_the_time( 'Y' ), 'w' => get_the_time( 'W' ) ), home_url() ), sprintf( __( 'Week %1$s', 'wplatte' ), get_the_time( esc_html_x( 'W', 'weekly archives date format', 'wplatte' ) ) ), false );
					else
		...
		elseif ( is_search() ) {
			...
			if ( is_paged() )
				$trail[] = '<a href="' . get_search_link() . '" title="' . sprintf( esc_attr__( 'Search results for &quot;%1$s&quot;', 'wplatte' ), esc_attr( get_search_query() ) ) . '">' . sprintf( __( 'Search results for &quot;%1$s&quot;', 'wplatte' ), esc_attr( get_search_query() ) ) . '</a>';
			else
				$trail[] = sprintf( __( 'Search results for &quot;%1$s&quot;', 'wplatte' ), esc_attr( get_search_query() ) );
		}
		...
		return apply_filters( 'WpLatteBreadcrumbsItems', $trail, $args );
	}
	 ...
	 * @return array List of items to be shown in the trail.
	 */
	public static function getBBpressItems( $args = array() ) {
		...
		return apply_filters( 'WpLatteBreadcrumbsBBpressItems', $trail, $args );
	}
	 ...
	 */
	public static function mapRewriteTags( $post_id = '', $path = '', $args = array() ) {
	 ...
	 */
	public static function getParents( $post_id = '', $path = '' ) {
	 ...
	 */
	public static function getTermParents( $parent_id = '', $taxonomy = '' ) {
	...
	public static function woocommerceBreadcrumbs($args)
	{
		...
		if($args['showHome']){
			...
			if(!is_shop()){
				$shopPage = get_page(wc_get_page_id('shop'));
		...
		foreach($trails as $i => $trail){
			...
			}elseif(empty($trail[1]) and count($trails) === ($i + 1)){
				$output .= '<span class="breadcrumb-end">';
			}

			if(!empty($trail[1]) and count($trails) !== ($i + 1)){
