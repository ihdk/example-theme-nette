 ...
 *
 * Modified by ait-themes.com from commit 00fdb9ea66
 */
...
class lessc {
	...
	protected function findImport($url) {
		foreach ((array)$this->importDir as $dir) {
			$full = $this->maybeSlashIt($dir) . $url;
	...
	// attempts to find the path of an import url, returns null for css files
	function findImportUrl($url)
	{
		$this->importUrl = (array) $this->importUrl;
		foreach ((array)$this->importDir as $i => $dir) {
			$full = $this->maybeSlashIt($dir) . $url;
			if ($this->fileExists($full) and isset($this->importUrl[$i])) {
				return $this->maybeSlashIt($this->importUrl[$i]) . $url;
			}
		}
	...
	protected function maybeSlashIt($path)
	{
		return $path . (substr(trim($path), -1) != '/' ? '/' : '');
	}
	...
	protected function isCss($path)
	{
		if(is_string($path) and strlen($path) > 4){
			return substr_compare(trim($path), '.css', -4, 4) === 0;
		}
		return false;
	}
	...
	protected function tryImport($importPath, $parentBlock, $out) {
		...
		if ($this->isCss($url) and !$this->importCss)
			return false;
	...
	protected function compileValue($value) {
		...
		case 'string':
			...
			if($this->isCss($content[0]) and !$this->importCss){
				$content[0] = $this->findImportUrl($content[0]);
			}
